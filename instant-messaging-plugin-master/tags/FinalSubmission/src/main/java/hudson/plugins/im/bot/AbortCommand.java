...
public class AbortCommand extends AbstractSingleJobCommand {
	 ...
	 */
	@Override
	protected CharSequence getMessageForJob(AbstractProject<?, ?> project, Sender sender, String[] args)
		...
		if (!aborted) {
			...
			// TODO: do build.doStop() instead of the following lines when
			// moving to core 1.489!
		...
		if (aborted) {
			...
			throw new CommandException(sender.getNickname() + ": " + " couldn't abort " + project.getFullDisplayName()
