...
public class Bot implements IMMessageListener {
	...
	/**
	 * Help command for the instant messaging bot.
	 *
	 */
	@Extension
	public static class HelpCommand extends BotCommand {
		 ...
		 * {@inheritDoc}
		 */
		public void executeCommand(Bot bot, IMChat groupChat, IMMessage message, Sender sender, String[] args)
				throws IMException {
	 ...
	 * 
	 * @param chat
	 *            An object standards for the chat room channel. Used to send
	 *            messages back to chat room.
	 * @param nick
	 *            The nick name of plug-in
	 * @param imServer
	 *            The name of the chat room channel
	 * @param commandPrefix
	 *            The prefix of command defined in the configuration page, like
	 *            "!jenkins"
	 * @param authentication
	 *            The authentication used by Jenkins to verify the identity of
	 *            the user.
	 */
	...
	/**
	 * Returns an identifier describing the Im account used to send the build
	 * command. E.g. the Jabber ID of the Bot.
	 */
	...
	/**
	 * {@inheritDoc} The real message handler.
	 */
			...
			try {
				if (!this.commandsAccepted) {
					...
					this.chat.sendMessage(
							s.getNickname() + " you're not a buddy of me. I won't take any commands from you!");
					return;
				}
			} catch (IMException e) {
				...
				try {
					...
					if (command != null) {
						if (isAuthenticationNeeded()) {
							ACL.impersonate(this.authentication.getAuthentication(),
									new NotReallyRoleSensitiveCallable<Void, IMException>() {
						...
						this.chat.sendMessage(s.getNickname() + " did you mean me? Unknown command" + cmd + "'\nUse '"
								+ this.commandPrefix + " help' to get help!");
					}
				} catch (Exception e) {
					LOGGER.warning(ExceptionHelper.dump(e));
				}
	...
	/**
	 * Returns the command or alias associated with the given name or
	 * <code>null</code>.
	 */
	 ...
	 * 
	 * @return the alias previously registered under this name or
	 *         <code>null</code> if no alias was registered by that name
	 *         previously
	 * @throws IllegalArgumentException
	 *             when trying to override a built-in command
	 */
	 ...
	 * @return the removed alias or <code>null</code> if no alias by that name
	 *         is registered
	 */
	...
	/**
	 * Returns a map of all currently defined aliases. The map is sorted by the
	 * alias name.
	 */
