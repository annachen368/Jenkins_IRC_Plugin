...
public class StatusCommand extends AbstractMultipleJobCommand {
	...
	protected CharSequence getMessageForJob(AbstractProject<?, ?> project) {
		...
		if (project.isDisabled()) {
			msg.append("(disabled) ");
			// a project which is building and additionally in queue should be
		...
		if (lastBuild != null) {
			msg.append("last build: ").append(lastBuild.getNumber()).append(" (").append(lastBuild.getTimestampString())
					.append(" ago): ").append(lastBuild.getResult()).append(": ")
