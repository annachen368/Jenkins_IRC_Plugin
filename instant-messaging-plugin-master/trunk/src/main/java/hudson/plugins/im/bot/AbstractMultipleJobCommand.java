...
abstract class AbstractMultipleJobCommand extends AbstractTextSendingCommand {
	 ...
	 * 
	 * @param projects
	 *            the list to which the projects are added
	...
	Pair<Mode, String> getProjects(Sender sender, String[] args, Collection<AbstractProject<?, ?>> projects)
		...
		if (args.length >= 2) {
			...
			mode = Mode.ALL;
			// don't show really all - could by quite many - but only the
			// top-level jobs
	...
	public String getHelp() {
		return " [<job>|-v <view>] - show the " + getCommandShortName()
				+ " of a specific job, jobs in a view or all jobs";
	}

	/**
	 * Collect all Projects within specific view.
	 * 
	 * @param toAddTo
	 *            The collection that stores the projects
	 * @param viewName
