...
public class PircListener extends ListenerAdapter<PircBotX> {
	...
	/**
	 * Controls a message sent from PircBotx when event has only or format
	 * indicator, message is not send it to other event listener, but modifies
	 * in the IRC plugin.
	 * 
	 * @param event
	 *            user's is received via PircBotx {@inheritDoc}
	 */
	@Override
		...
		if (event.getMessage().contains(onlyIndicator)) {
			...
			for (MessageListener l : this.msgListeners) {
				if (l.target.equals(event.getChannel().getName())) {
					l.listener.onMessage(
							new IMMessage(event.getUser().getNick(), event.getChannel().getName(), event.getMessage()));
				}
			}
		}
	}
	...
	/**
	 * Gets user names from message passed by PircBotx (with "only" flag).
	 * 
	 * @param msg
	 * @param noUser
	...
	/**
	 * Controls a private message sent from PircBotx, but when the only
	 * indicator is sent up then the IRC plugin controls its message.
	 * 
	 * @param event
	 *            user's is received via PircBotx {@inheritDoc}
	 */
	 ...
	 * 
	 * @param event
	 *            user's is received via PircBotx
	 */
	@Override
	...
	private static final class MessageListener {
		...
		public MessageListener(String expectedMessageTarget, String expectedMessageSender, IMMessageListener listener) {
			this.target = expectedMessageTarget;
